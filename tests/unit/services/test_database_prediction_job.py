...
class TestPredictionJob(unittest.TestCase):
    ...
    def test__get_prediction_jobs_query_results(
        self, add_description_mock, data_interface_mock,
    ):
        # Arrange
        data_interface_mock.get_instance.return_value.exec_sql_query.return_value = pd.DataFrame([self.prediction_job])
